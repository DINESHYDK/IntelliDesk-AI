            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  ticket.priority === 'P1' && ticket.status !== 'Resolved' && 'border-l-2 border-l-destructive bg-destructive/5'
                )}
              >
                {/* Desktop Row */}
                <div className='hidden lg:grid lg:grid-cols-12 gap-4 px-6 py-4 items-center'>
                  ...
                  <div className='col-span-5 min-w-0'>
                    <div className='flex items-center gap-2 mb-1'>
                      ...
                      {ticket.status === 'Open' && ticket.tone?.toLowerCase().includes('urgent') && (
                         <span className='w-2 h-2 rounded-full bg-orange-500 animate-pulse' title='Response received - Action needed' />
                      )}
                    </div>
                    <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                      <span className='truncate max-w-[200px]'>{ticket.sender_email}</span>
                      {ticket.customer_domain && (
                          <>
                           <span className='text-muted-foreground/60'></span>
                           <span className='truncate'>{ticket.customer_domain}</span>
                          </>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Status */}
                  <div className='col-span-2'>
                     <span className={cn('text-xs font-medium px-2 py-1 rounded inline-block',
                         ticket.status === 'New' ? 'text-blue-500 bg-blue-500/10' :
                         ticket.status === 'Open' ? 'text-orange-500 bg-orange-500/10' :
                         ticket.status === 'Resolved' ? 'text-green-500 bg-green-500/10' :
                         'text-slate-500 bg-slate-500/10'
                     )}>
                         {ticket.status}
                     </span>
                     {ticket.confidence_score > 0.8 && ticket.status === 'New' && (
       ...
       {totalPages > 1 && (
        <div className='flex items-center justify-between p-4 border-t border-border bg-muted/10'>
          <p className='text-xs text-muted-foreground'>
            Showing {Math.min(displayedTickets.length, tickets.length)} of {tickets.length} tickets
          </p>
          <div className='flex items-center gap-2'>
            <Button
              variant='outline'
