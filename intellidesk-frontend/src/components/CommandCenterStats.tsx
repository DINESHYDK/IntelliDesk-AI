...
export function CommandCenterStats({ tickets, isLoading = false }: CommandCenterStatsProps) {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-28 rounded-xl bg-slate-800/50 border border-slate-700/50 animate-shimmer" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 animate-fade-in">
      ...
      <div className="col-span-1 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm hover:border-slate-600 transition-all">
        <div className="flex items-center gap-2 mb-3">
          <Activity className="w-4 h-4 text-blue-400" />
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Priority Pulse</span>
        </div>
        <div className="grid grid-cols-4 gap-2">
          ...
          {/* P3 */}
          <div className="text-center">
            <div className="w-full aspect-square rounded-lg bg-blue-500/20 border border-blue-500/40 flex items-center justify-center mb-1">
              <span className="text-lg font-bold text-blue-400">{metrics.priorityCounts.P3}</span>
            </div>
            <span className="text-[10px] font-medium text-blue-400">P3</span>
          </div>
          {/* P4 */}
          <div className="text-center">
            <div className="w-full aspect-square rounded-lg bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center mb-1">
              <span className="text-lg font-bold text-emerald-400">{metrics.priorityCounts.P4}</span>
            </div>
            <span className="text-[10px] font-medium text-emerald-400">P4</span>
          </div>
        </div>
      </div>
      ...
      <div className="col-span-1 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm hover:border-slate-600 transition-all">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Customer Mix</span>
        </div>
        <div className="space-y-2.5">
          ...
          {/* Bronze */}
          <div className="flex items-center gap-2">
            <span className="w-12 text-xs font-medium text-orange-300 flex items-center gap-1">
              <Award className="w-3 h-3" /> Bronze
            </span>
            <div className="flex-1">
              <MiniProgress value={metrics.tierCounts.Bronze} max={metrics.totalTickets} color="bg-orange-600" />
            </div>
            <span className="w-6 text-xs font-bold text-orange-300 text-right">{metrics.tierCounts.Bronze}</span>
          </div>
        </div>
      </div>
      ...
      <div className="col-span-1 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm hover:border-slate-600 transition-all">
        ...
        {/* Progress bar */}
        <div className="h-2 w-full bg-slate-700/50 rounded-full overflow-hidden flex">
          <div 
      ...
      <div className="col-span-1 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm hover:border-slate-600 transition-all">
        ...
        <div className="flex items-center gap-3">
            ...
            <svg className="w-14 h-14 -rotate-90">
              ...
              <circle
                ...
                className={cn(
                  ...
                  metrics.avgConfidence >= 80 ? 'text-emerald-400' : 'text-amber-400'
                )}
              />
            </svg>
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Avg. Classification</p>
          </div>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        metrics.slaMetrics.breached > 0 
          ? 'bg-red-500/10 border border-red-500/40 hover:border-red-400' 
          : 'bg-slate-800/50 border border-slate-700/50 hover:border-slate-600'
      )}>
        <div className="flex items-center gap-2 mb-3">
          <Clock className="w-4 h-4 text-red-400" />
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">SLA Alert Zone</span>
          {metrics.slaMetrics.breached > 0 && <PulseDot color="bg-red-500" />}
        </div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-2 rounded-lg bg-amber-500/10 border border-amber-500/30">
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className="text-[10px] text-amber-400 font-medium">NEAR BREACH</span>
            </div>
            <span className="text-xl font-bold text-amber-400">{metrics.slaMetrics.nearBreach}</span>
            <span className="text-[10px] text-amber-400/70 ml-1">&lt;1h left</span>
          </div>
          {/* Breached */}
          <div className="p-2 rounded-lg bg-red-500/10 border border-red-500/30">
            <div className="flex items-center gap-1 mb-1">
              <AlertTriangle className="w-3 h-3 text-red-400 animate-pulse" />
              <span className="text-[10px] text-red-400 font-medium">BREACHED</span>
            </div>
            <span className="text-xl font-bold text-red-400">{metrics.slaMetrics.breached}</span>
            <span className="text-[10px] text-red-400/70 ml-1">overdue</span>
          </div>
        </div>
      </div>
